...
#include <deal.II/lac/generic_linear_algebra.h>		//par
...
namespace thesis
{
    ...
    private:
      ...
      /*!Newton-Raphson algorithm looping over all newton iterations*/			//par
      void solve_nonlinear_newton(const parameters::AllParameters &param
                                  ,LA::MPI::BlockVector/*<double>*/ &solution_delta);

      /*!Solve the linear system as assembled via assemble_system()*/			//par
      std::pair<unsigned int,double> solve_linear_sys (const parameters::AllParameters &param,
                                                       LA::MPI::BlockVector/*<double>*/ & newton_update);

      /*!Set hanging node and Dirichlet constraints.*/
      void make_constraints(unsigned int &itr);

      /*!Assemble the linear system for the elasticity problem*/			//par
      void assemble_system (const parameters::AllParameters &param,
                            LA::MPI::BlockVector/*<double>*/ & update);
      ...
      MPI_Comm 					mpi_com;				//par

      dealii::parallel::distributed::Triangulation<dim>   triangulation_m;		//par
      ...
      dealii::TrilinosWrappers::BlockSparsityPattern      sparsity_pattern_m;			//par //??
      LA::MPI::BlockSparseMatrix 			tangent_matrix_m;
      LA::MPI::BlockVector       			solution_m;
      LA::MPI::BlockVector				system_rhs_m,newton_update;

      dealii::ConditionalOStream                pcout;					//par
      ...
      static const unsigned int n_blocks_m = 2;//1;
...
//      std::vector<dealii::types::global_dof_index> dofs_per_block_m;	//par
//      std::vector<dealii::IndexSet>                     owned_partitioning;			//par
//      std::vector<dealii::IndexSet>                     relevant_partitioning;
      std::vector<dealii::IndexSet>                     partition;			//par
      std::vector<dealii::IndexSet>                     partition_relevant;
